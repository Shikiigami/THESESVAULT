  ...
  <nav>
    ...
    <section class="section dashboard">
      <div class="row">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Full Text Requests Data</h5>
              ...
              <table class="table datatable table-hover">
                ...
                <tbody>
                    @foreach($fullRequests as $request)
                    <tr>
                        <td scope="row">{{$request->full_userId->name}}</td>
                        <td scope="row">{{$request->full_userId->email}}</td>
                        <td scope="row">{{$request->full_researchId->callno}}</td>
                        @php
                        $userRole = ($request->full_userId->role == 'user') ? 'Student' : (($request->full_userId->role == 'faculty') ? 'faculty' : 'N/A');
                        @endphp
                        <td >{{$userRole}}</td>
                        @if (empty($request->full_userId->college->college_name))
                        <td></td>
                        @else
                            <td>{{$request->full_userId->college->college_name}}</td>
                        @endif
                        <td >{{$request->full_userId->program}}</td>
                        <td >{{$request->purpose}}</td>
                        <td style="vertical-align: middle!important;text-align: center;  ">
                        <div style="display: inline-flex; gap: 5px; justify-content: center;">
                        @if($request->requests_stat =='Pending')
                        <form action="{{ route('fullrequest.approved', ['id' => $request->id]) }}" method="POST">
                            @csrf
                            @method('PUT')
                        ...
                        <form action="{{ route('fullrequest.declined', ['id' => $request->id]) }}" method="POST">
                            @csrf
                            @method('PUT')
                        ...
                        @elseif ($request->requests_stat =='Approved')
